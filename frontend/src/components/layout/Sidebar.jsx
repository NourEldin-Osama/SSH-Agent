...
import { ChevronLeft, ChevronRight, Pencil, Plus, Trash2, History, CalendarDays, Command } from 'lucide-react'
      ...
      {open && (
        <div className="flex-1 overflow-y-auto">
          <div className="px-3 py-2 border-b border-gray-800 flex items-center justify-between">
            <span className="text-xs text-gray-500 uppercase font-medium inline-flex items-center gap-1"><History className="w-3.5 h-3.5" />Sessions</span>
            <button
          ...
          {sessions.map((session) => (
            ...
            >
              ...
              <div className="text-xs text-gray-600 mt-0.5 flex items-center justify-between">
                <span className="inline-flex items-center gap-1"><CalendarDays className="w-3 h-3" />{new Date(session.created_at).toLocaleString()}</span>
                <span className="inline-flex items-center gap-1"><Command className="w-3 h-3" />{session.command_count || 0} cmds</span>
              </div>
            </div>
          ))}
